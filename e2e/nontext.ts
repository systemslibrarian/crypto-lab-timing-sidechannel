 ...
 *
 * The first half is the live one in this repo. It checks a claim the
 * stylesheet makes about itself: `main.css` defines `--border-strong` with a
 * comment asserting specific ratios — "4.13:1 on --panel, 3.76:1 on --bg,
 * 3.25:1 on --bg-soft" in the light theme, "3.42:1 / 3.76:1 / 3.26:1" in the
 * dark — and applies it to the shared `button, input` rule precisely because
 * those controls are filled with `--panel`, the colour of the card behind them
 * at 1.00:1, so the border is the only thing that makes them visible. A
 * comment is not evidence: it was written for the flat `--panel` fill, while
 * every primary `button` on this page actually paints a
 * `linear-gradient(160deg, color-mix(in srgb, var(--accent) 24%, var(--panel)),
 * var(--panel))` over a `.panel` that is itself a `linear-gradient(165deg, …)`,
 * and `button.secondary` (New secret, Run all four modes) drops back to the
 * flat fill. This oracle measures the composite each control is really drawn
 * on, at every driven state, including `:hover` — which persists after a click
 * and is the state a reader occupies the instant after pressing Run.
 *
 * The second half is INERT here today, and that is a measured fact rather than
 * an assumption: `styles/main.css` declares no `content:` rule at all — this
 * lab's non-colour cues (the ⚠ / ✓ / • verdict icons, the "⚠ leak" / "✓ safe"
 * board tags, the ␣/character glyphs of the reveal diff) are real text nodes
 * the arithmetic walk in `contrast.ts` measures directly. The half still runs
 * at every driven state because it costs nothing and because the first
 * `::before`/`::after` glyph anyone adds would otherwise be invisible to every
 * oracle in this gate.
 *
 ...
 *    the wrong surface, and `escaped` is reported so triage can see it. With
 *    no pseudo-content in this lab's stylesheet, no finding can land in that
 *    category today; the reporting stays so the first positioned glyph added
 *    is triaged rather than trusted.
 *
...
export async function auditNonText(page: Page, within = 'body *'): Promise<NonTextFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     * …)` washes — the accent at 12% 8% and accent-2 at 92% 0% — over
     * `var(--bg)`. Every
     ...
     * (`::before` and `::after`) on every element on the page, and a driven
     * state renders one `<span>` per recovery slot and reveal-diff character
     * plus every cell of the sweep, per-position and board tables. Without the
    ...
    const CONTROL = [
      ...
      // control?" test below. This lab's Section 5 `.links a` chips are pill-shaped
